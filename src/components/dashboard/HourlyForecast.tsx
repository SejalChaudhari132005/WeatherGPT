...
import { HourlyForecastItem } from '../../data/mockWeather';
    ...
    <div className="p-3.5 sm:p-4 rounded-2xl sm:rounded-3xl bg-slate-900 text-white shadow-xl space-y-2.5 sm:space-y-3 font-['Arimo']">
      <div className="flex items-center justify-between text-xs font-extrabold px-1 gap-2">
        <div className="flex items-center gap-1.5 text-slate-300 min-w-0">
          ...
          <span className="truncate">Hourly Forecast</span>
        </div>
      ...
      <div className="flex gap-2 sm:gap-2.5 overflow-x-auto snap-x snap-mandatory py-1 no-scrollbar scrollbar-none">
        {hourly.map((item, idx) => (
          ...
          >
            ...
            <span className="text-lg sm:text-xl my-1">{item.icon}</span>
            <span className="text-xs sm:text-sm font-black">{item.temp}°C</span>
            <span className={`text-[8px] sm:text-[9px] font-bold mt-1 px-1.5 py-0.5 rounded-full ${item.highlight ? 'bg-amber-400 text-slate-900' : 'bg-slate-700 text-sky-200'}`}>
              {item.rainProb}% rain
            </span>
          </div>
        ))}
