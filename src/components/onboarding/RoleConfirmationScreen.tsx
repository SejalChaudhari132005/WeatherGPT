...
import { ROLE_OPTIONS } from './RoleSelectionScreen';

export const RoleConfirmationScreen: React.FC = () => {
  const { username, role } = useProfile();
  const { goToStep } = useAuth();

  const selectedOption = ROLE_OPTIONS.find((r) => r.id === role) || ROLE_OPTIONS[0];
    ...
    <div className="min-h-screen bg-[#F4F7FC] flex items-center justify-center p-4 sm:p-6 md:p-8 relative">
      <div className="w-full max-w-md bg-white sm:rounded-3xl sm:shadow-2xl sm:border sm:border-slate-200/80 p-6 sm:p-8 flex flex-col justify-between min-h-[85vh] sm:min-h-[520px] text-center transition-all">
        <div className="pt-4 space-y-4">
          ...
          <div className="my-4 inline-flex p-5 rounded-3xl bg-slate-50 border border-slate-200 shadow-md text-5xl animate-bounce">
            {selectedOption.icon}
          </div>

          <h2 className="text-3xl font-black text-slate-900 tracking-tight">
            Great, {username || 'friend'}!
          </h2>

          <p className="text-lg font-bold text-sky-700">
            We'll personalize WeatherGPT for {selectedOption.title.toLowerCase()}s.
          </p>
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200/80 shadow-2xs max-w-xs mx-auto my-3 text-left">
            ...
            <p className="text-xs font-semibold text-slate-700 leading-relaxed">
              {selectedOption.advisoryPreview}
            </p>
          </div>
        </div>
